...
- (void)validateLayout
{
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2) {
            ...
            if ([button.titleLabel.text length]>7) {
                
                self.containerView.frame = CGRectMake(left, top, CONTAINER_WIDTH, height+10);
                self.titleLabel.textAlignment = NSTextAlignmentLeft;
                self.messageLabel.textAlignment = NSTextAlignmentLeft;
                self.messageLabel.frame = CGRectMake(CONTENT_PADDING_LEFT, 40, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, 20);
                button.frame = CGRectMake(0, y-40, CONTAINER_WIDTH, BUTTON_HEIGHT);
                button = self.buttons[1];
                button.frame = CGRectMake(0, y+10, CONTAINER_WIDTH, BUTTON_HEIGHT);
            }
            else{
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
	...
	switch (item.type) {
		case SIAlertViewButtonTypeCancel:
			normalImage = [UIImage imageNamed:@"button-cancel"];
			highlightedImage = [UIImage imageNamed:@"button-cancel-d"];
		...
		case SIAlertViewButtonTypeDestructive:
			normalImage = [UIImage imageNamed:@"button-destructive"];
			highlightedImage = [UIImage imageNamed:@"button-destructive-d"];
		...
		default:
			normalImage = [UIImage imageNamed:@"button-default"];
			highlightedImage = [UIImage imageNamed:@"button-default-d"];
